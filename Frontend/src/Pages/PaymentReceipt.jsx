...
const PaymentReceipt = () => {
  ...
  const downloadPDF = () => {
    ...
    // Footer message
    doc.setFontSize(10);
    doc.text("Thank you for your purchase!", pageWidth / 2, y + 10, null, null, "center");
  ...
  return (
    <div className="max-w-2xl mx-auto mt-10 bg-gray-100 shadow-lg rounded-lg p-8">
      <div className="flex flex-col items-center mb-4">
        <img src={assets.logo} alt="NeonBrush Logo" className="w-10 h-10 mb-2" />
      ...
      <div className="mt-6 space-y-4">
        ...
        {cartItems.map((item) => (
          <div key={item.artId} className="flex justify-between py-2 border-b">
            ...
            <p className="text-right text-gray-900 font-semibold">Rs.{item.price}</p>
          </div>
        ))}
      ...
      <div className="flex justify-end space-x-4 mt-6">
        <button onClick={() => navigate("/products")} className="py-3 px-6 rounded-lg">
          <IoChevronBackCircleOutline className="text-black text-2xl" />
        </button>

        <button onClick={downloadPDF} className="bg-sky-950 text-white py-3 px-6 rounded-lg hover:bg-sky-800">
          Download PDF
        </button>
      </div>
    </div>
